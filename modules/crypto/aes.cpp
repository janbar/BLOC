...
/*
 * The round constant word array contains the values given by x to the power
 * (i-1) being powers of x (x is denoted as {02}) in the field GF(2^8).
 *
...
/*
 * The KeyExpansion function produces Nb(Nr+1) round keys. The round keys are
 * used in each round to decrypt the states.
 */
...
/*
 * The AddRoundKey function adds the round key to state. The round key is added
...
/*
 * The Multiply function is used to multiply numbers in the field GF(2^8).
 * Note: The last call to xtime() is unneeded, but often ends up generating a
 * smaller binary. The compiler seems to be able to vectorize the operation
 * better this way.
 */
 ...
 */
static void Cipher(const AES_params* params, state_t* state, const uint8_t* RoundKey)
{
  uint8_t round = 0;

  // Add the First round key to the state before starting the rounds.
  AddRoundKey(params, 0, state, RoundKey);
  ...
  for (round = 1; ; ++round)
  {
    ...
    MixColumns(state);
    AddRoundKey(params, round, state, RoundKey);
  }
  // Add round key to last round
  AddRoundKey(params, params->nr, state, RoundKey);
}

static void InvCipher(const AES_params* params, state_t* state, const uint8_t* RoundKey)
{
  uint8_t round = 0;
...
{
  ctx->params = &AES_CONFIG_PARAMS[conf];
}

/*
 * Set the Key.
 * Required length is 128, 192 or 256 bits, depending of AES config.
 */
void AES_ctx_set_key(AES_ctx* ctx, const uint8_t* key)
{
  KeyExpansion(ctx->params, ctx->round_key, key);
}

/*
 * Set initial IV, used in CBC mode.
 * Required length is the AES block length (128 bits).
 */
...
{
  Cipher(ctx->params, (state_t*)buf, ctx->round_key);
}

void AES_ECB_decrypt(const AES_ctx* ctx, uint8_t* buf)
{
  InvCipher(ctx->params, (state_t*)buf, ctx->round_key);
}
