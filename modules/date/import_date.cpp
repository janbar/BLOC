  ...
  try
  {
    ...
    case 1: /* date ( string, format ) */
    {
      ...
      if (strptime(args[0]->literal(ctx).c_str(), args[1]->literal(ctx).c_str(), &_tm) == nullptr)
        throw RuntimeError(EXC_RT_USER_S, "Invalid date format.");
      /* string is local time (no tz info) */
      time_t tt = mktime(&_tm);
      if (tt == INVALID_TIME)
        throw RuntimeError(EXC_RT_USER_S, "The system does not support this date range.");
          ...
          )
{
  ...
  case date::Add_unit:
  {
    ...
    if (unit < 0)
      throw RuntimeError(EXC_RT_USER_S,"Invalid name for time unit.");
