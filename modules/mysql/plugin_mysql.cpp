...
/*
 * Create the module MySQLImport
 */
PLUGINCREATOR(MySQLPlugin)
...
void MySQLPlugin::declareInterface(PLUGIN_INTERFACE * interface)
{
  interface->name = "mysql";
  interface->method_count = sizeof(MySQL::methods) / sizeof(PLUGIN_METHOD);
  interface->methods = MySQL::methods;
  interface->ctors_count = sizeof(MySQL::ctors) / sizeof(PLUGIN_CTOR);
  interface->ctors = MySQL::ctors;
}

void * MySQLPlugin::createObject(int ctor_id, bloc::Context& ctx, const std::vector<bloc::Expression*>& args)
{
  switch (ctor_id)
  {
  case 0: /* copy ctor */
  {
    MySQL::Handle * h = nullptr;
    bloc::Value& a0 = args[0]->value(ctx);
    if (a0.isNull() || (h = static_cast<MySQL::Handle*>(a0.complex()->instance())) == nullptr)
       throw RuntimeError(EXC_RT_OTHER_S, "Invalid arguments.");
    MySQL::Handle * n = new MySQL::Handle();
    if (h->isOpen())
    {
  ...
  case 1:
  {
    ...
    if (a0.isNull() || a1.isNull() || a2.isNull() || a3.isNull() || a4.isNull())
      throw RuntimeError(EXC_RT_OTHER_S, "Invalid arguments.");
    MySQL::Handle * h = new MySQL::Handle();
...
void MySQLPlugin::destroyObject(void * object)
{
  MySQL::Handle * h = static_cast<MySQL::Handle*>(object);
  delete h;
}

bloc::Value * MySQLPlugin::executeMethod(
...
{
  MySQL::Handle * h = static_cast<MySQL::Handle*>(object_this.instance());
...
MySQL::Handle::Bindings::Bindings(unsigned size)
{
  reset(size);
}

MySQL::Handle::Bindings::~Bindings()
{
  free_all();
}

void MySQL::Handle::Bindings::reset(unsigned size)
{
  free_all();
...
void MySQL::Handle::Bindings::free_all()
{
  for (MYSQL_BIND& bind : _bindings)
  {
    if (bind.buffer)
...
unsigned long MySQL::Handle::Bindings::alloc_buffer(MYSQL_BIND& b, unsigned long len)
{
...
void MySQL::Handle::Bindings::free_buffer(MYSQL_BIND& b)
{
  if (b.buffer)
    delete [] (char*)b.buffer;
  b.buffer = nullptr;
...
void MySQL::Handle::bind_args(Bindings& bindings, bloc::Tuple& args)
{
...
void MySQL::Handle::unbind_args(Bindings& bindings)
{
...
int MySQL::Handle::open(const std::string& host,
                        const std::string& user,
...
int MySQL::Handle::autocommit(bool mode)
{
...
int MySQL::Handle::fetchall(MYSQL_STMT * stmt, bloc::Collection ** rs)
{
  int r = 0;
...
int MySQL::Handle::query(const std::string& str, bloc::Collection ** rs)
{
  int r = 0;
...
int MySQL::Handle::query(const std::string& str, bloc::Tuple& args, bloc::Collection ** rs)
{
  int r = 0;
  MYSQL_STMT * stmt = mysql_stmt_init(_db);
  if (stmt)
...
int MySQL::Handle::exec(const std::string& str)
{
  int r = 0;
  MYSQL_STMT * stmt = mysql_stmt_init(_db);
  if (stmt)
...
int MySQL::Handle::exec(const std::string& str, bloc::Tuple& args)
{
  int r = 0;
  MYSQL_STMT * stmt = mysql_stmt_init(_db);
  if (stmt)
...
int MySQL::Handle::prepare(const std::string& str)
{
  if (_stmt)
    finalize();
...
int MySQL::Handle::execute(bloc::Tuple& args)
{
  int r = 0;
  if (_stmt == nullptr)
    return r;
...
int MySQL::Handle::header(bloc::Collection ** hd)
{
  if (_stmt == nullptr)
...
int MySQL::Handle::fetch(bloc::Tuple ** row)
{
  if (_stmt == nullptr)
    throw RuntimeError(EXC_RT_OTHER_S, "No query in progress");
  /* initialize bindinds */
