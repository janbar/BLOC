...
TEST_CASE("operator I + N")
{
  ...
  REQUIRE( fequal(*(e->value(ctx).numeric()), 51140.0123456) );
  delete e;
}
...
TEST_CASE("operator I - N")
{
  ctx.reset("7687 - 43453.0123456");
  Expression * e = ctx.parseExpression();
  REQUIRE( fequal(*(e->value(ctx).numeric()), -35766.0123456) );
  delete e;
}
...
TEST_CASE("operator I * N")
{
  ctx.reset("7687 * 43453.0123456");
  Expression * e = ctx.parseExpression();
  REQUIRE( fequal(*(e->value(ctx).numeric()), 334023305.9006272) );
  delete e;
}
...
TEST_CASE("operator I / N")
{
  ...
  Expression * e = ctx.parseExpression();
  REQUIRE( fequal(*(e->value(ctx).numeric()), 5.652781347) );
  delete e;
}
...
TEST_CASE("operator N % I")
{
  ...
  Expression * e = ctx.parseExpression();
  REQUIRE( fequal(*(e->value(ctx).numeric()), 5018.0123456) );
  delete e;
}
...
TEST_CASE("operator I ** N (POWER)")
{
  ...
  REQUIRE( fequal(*(e->value(ctx).numeric()), 489777568.25277) );
  delete e;
...
TEST_CASE("operator N ** I (POWER)")
{
  ...
  Expression * e = ctx.parseExpression();
  REQUIRE( fequal(*(e->value(ctx).numeric()), 451239450.633787) );
  delete e;
}
