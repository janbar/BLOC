...
Value& COSExpression::value(Context & ctx) const
{
  ...
  case Type::INTEGER:
    if (val.isNull())
      return val;
    v = Value(Numeric(std::cos(*val.integer())));
    break;
  case Type::NUMERIC:
    if (val.isNull())
      return val;
  ...
  case Type::IMAGINARY:
    ...
    {
      auto z = std::cos(IMAGINARY_TO_COMPLEX(*val.imaginary()));
...
COSExpression * COSExpression::parse(Parser& p, Context& ctx)
{
  ...
  try
  {
    TokenPtr t = p.pop();
    if (t->code != '(')
      throw ParseError(EXC_PARSE_FUNC_ARG_NUM_S, KEYWORDS[FUNC_COS], t);
    args.push_back(ParseExpression::expression(p, ctx));
    if (!ParseExpression::typeChecking(args.back(), Type::NUMERIC, p, ctx))
      throw ParseError(EXC_PARSE_FUNC_ARG_TYPE_S, KEYWORDS[FUNC_COS], t);
    assertClosedFunction(p, ctx, FUNC_COS);
    return new COSExpression(std::move(args));
  }
  catch (ParseError& pe)
  {
