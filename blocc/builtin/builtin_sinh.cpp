...
Value& SINHExpression::value(Context & ctx) const
{
  ...
  case Type::INTEGER:
    if (val.isNull())
      return val;
    v = Value(Numeric(std::sinh(*val.integer())));
    break;
  case Type::NUMERIC:
    if (val.isNull())
      return val;
...
SINHExpression * SINHExpression::parse(Parser& p, Context& ctx)
{
  ...
  try
  {
    TokenPtr t = p.pop();
    if (t->code != '(')
      throw ParseError(EXC_PARSE_FUNC_ARG_NUM_S, KEYWORDS[FUNC_SINH], t);
    args.push_back(ParseExpression::expression(p, ctx));
    if (!ParseExpression::typeChecking(args.back(), Type::NUMERIC, p, ctx))
      throw ParseError(EXC_PARSE_FUNC_ARG_TYPE_S, KEYWORDS[FUNC_SINH], t);
