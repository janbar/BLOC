...
Value& LOGExpression::value(Context & ctx) const
{
  ...
  case Type::IMAGINARY:
    ...
    {
      auto z = std::log(IMAGINARY_TO_COMPLEX(*val.imaginary()));
...
LOGExpression * LOGExpression::parse(Parser& p, Context& ctx)
{
  ...
  try
  {
    TokenPtr t = p.pop();
    if (t->code != '(')
      throw ParseError(EXC_PARSE_FUNC_ARG_NUM_S, KEYWORDS[FUNC_LOG], t);
    args.push_back(ParseExpression::expression(p, ctx));
    if (!ParseExpression::typeChecking(args.back(), Type::NUMERIC, p, ctx))
      throw ParseError(EXC_PARSE_FUNC_ARG_TYPE_S, KEYWORDS[FUNC_LOG], t);
