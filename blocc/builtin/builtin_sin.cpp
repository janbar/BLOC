...
Value& SINExpression::value(Context & ctx) const
{
  ...
  case Type::INTEGER:
    if (val.isNull())
      return val;
    v = Value(Numeric(std::sin(*val.integer())));
    break;
  case Type::NUMERIC:
    if (val.isNull())
      return val;
    v = Value(Numeric(std::sin(*val.numeric())));
    break;
  case Type::IMAGINARY:
    if (val.isNull())
      return val;
...
SINExpression * SINExpression::parse(Parser& p, Context& ctx)
{
  ...
  try
  {
    TokenPtr t = p.pop();
    if (t->code != '(')
      throw ParseError(EXC_PARSE_FUNC_ARG_NUM_S, KEYWORDS[FUNC_SIN], t);
    args.push_back(ParseExpression::expression(p, ctx));
    if (!ParseExpression::typeChecking(args.back(), Type::NUMERIC, p, ctx))
      throw ParseError(EXC_PARSE_FUNC_ARG_TYPE_S, KEYWORDS[FUNC_SIN], t);
    assertClosedFunction(p, ctx, FUNC_SIN);
    return new SINExpression(std::move(args));
  }
  catch (ParseError& pe)
  {
