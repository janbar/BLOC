...
Value& ASINExpression::value(Context & ctx) const
{
  ...
  case Type::IMAGINARY:
    ...
    {
      auto z = std::asin(IMAGINARY_TO_COMPLEX(*val.imaginary()));
...
ASINExpression * ASINExpression::parse(Parser& p, Context& ctx)
{
  ...
  try
  {
    TokenPtr t = p.pop();
    if (t->code != '(')
      throw ParseError(EXC_PARSE_FUNC_ARG_NUM_S, KEYWORDS[FUNC_ASIN], t);
    args.push_back(ParseExpression::expression(p, ctx));
    if (!ParseExpression::typeChecking(args.back(), Type::NUMERIC, p, ctx))
      throw ParseError(EXC_PARSE_FUNC_ARG_TYPE_S, KEYWORDS[FUNC_ASIN], t);
    assertClosedFunction(p, ctx, FUNC_ASIN);
    return new ASINExpression(std::move(args));
  }
  catch (ParseError& pe)
  {
