...
const Type& OpGEExpression::type(Context& ctx) const
{
...
Value& OpGEExpression::value(Context& ctx) const
{
...
std::string OpGEExpression::unparse(Context&ctx) const
{
  std::string str;
  if (arg2 == nullptr)
    str.append(Operator::OPVALS[Operator::OP_GE]).append(arg1->unparse(ctx));
  else
    str.append("(").append(arg1->unparse(ctx)).append(" ")
            .append(Operator::OPVALS[Operator::OP_GE]).append(" ")
