...
OpXORExpression::~OpXORExpression()
{
  if (arg2)
    delete arg2;
  if (arg1)
...
const Type& OpXORExpression::type(Context& ctx) const
{
...
Value& OpXORExpression::value(Context& ctx) const
{
...
std::string OpXORExpression::unparse(Context&ctx) const
{
  std::string str;
  if (arg2 == nullptr)
    str.append(Operator::OPVALS[Operator::OP_XOR]).append(arg1->unparse(ctx));
  else
    str.append("(").append(arg1->unparse(ctx)).append(" ")
            .append(Operator::OPVALS[Operator::OP_XOR]).append(" ")
