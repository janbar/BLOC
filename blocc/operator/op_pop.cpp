...
OpPOPExpression::~OpPOPExpression()
{
  if (arg2)
    delete arg2;
  if (arg1)
...
const Type& OpPOPExpression::type(Context& ctx) const
{
...
Value& OpPOPExpression::value(Context& ctx) const
{
...
std::string OpPOPExpression::unparse(Context&ctx) const
{
  std::string str;
  if (arg2 == nullptr)
    str.append(Operator::OPVALS[Operator::OP_POP]).append(arg1->unparse(ctx));
  else
    str.append("(").append(arg1->unparse(ctx)).append(" ")
            .append(Operator::OPVALS[Operator::OP_POP]).append(" ")
